...
import { useAuth } from '@/views/auth/useAuth' // ✅ 전역 auth 상태 사용

const router = useRouter()
const { isAuthed, logout } = useAuth()         // ✅ authed 플래그 + 로그아웃 액션

function goLogin() {
  router.push({ name: 'login' })               // 라우트 이름 프로젝트에 맞게
}

function doLogout() {
  logout()                                      // ✅ UI 상태 로그아웃(authed 플래그 해제)
  router.push({ name: 'home' })                 // 원하는 곳으로
}
</script>
...
.auth-btn {
  ...
  text-align: center; font-size: 14px; font-weight: 600; line-height: 1.2;
  cursor: pointer; text-decoration: none;
  box-shadow: 0 1px 2px rgba(0,0,0,0.08);
  transition: background-color .15s ease, filter .15s ease, transform .02s ease;
}
.auth-btn:hover { filter: brightness(.97); background-color: #fff; }
